"""
Watson Discovery V1 API のトレーニング用 CSV ファイル出力
"""
...
def convert_to_csv(file_name: str, queries: List) -> None:
    ...
    columns = ["num_queries", "natural_language_query", "num_examples", "examples_document_id", "examples_cross_reference", "examples_relevance"]  # noqa: E501
    ...
    for i in range(len(queries)):
        ...
        for j in range(len(examples)):
            df_data = pd.DataFrame(
                data=[[i, nl_query, j, examples[j]["document_id"], examples[j]["cross_reference"], examples[j]["relevance"]]],  # noqa: E501
                columns=columns)
